...
void dijkstra_with_priorities(Graph* g, int start_idx, int end_idx, Destination* dests, int dest_count, 
    ...
    // NEW: Calculate optimal baseline distance (no waypoints) for threshold comparison
    ...
    // NEW: Store all waypoints in order for final path reconstruction
    ...
    for (int waypoint_idx = 0; waypoint_idx < dest_count; waypoint_idx++) {
        ...
        // NEW: Check if adding this waypoint would exceed threshold
    ...
    // Final segment from last waypoint (or start if all skipped) to end_idx
